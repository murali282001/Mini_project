...
const createId = (() => {
  ...
  return () => {
    fallbackCounter += 1;
    return `expmon-${Date.now().toString(16)}-${Math.random().toString(16).slice(2)}-${fallbackCounter}`;
  };
})();
...
function boot() {
  try {
    ...
    console.error("Failed to render Expense Monitor", err);
    const app = document.getElementById("app");
    if (app) {
      app.innerHTML = `
        <section class="app-shell">
          <div class="card">
            ...
            <p class="muted">An unexpected error prevented the dashboard from loading. Please refresh the page to try again.</p>
          </div>
        </section>
      `;
    }
